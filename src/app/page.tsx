...
import { groupColors, seedItems, type Item } from "@/data/inventory";
  ...
  return (
    <div className={dark ? "app dark" : "app"}>
      ...
      <main className="main-shell">
        ...
        {active === "dashboard" && <Dashboard totals={totals} items={items} greeting={greeting} voiceText={voiceText} listening={listening} startVoice={startVoice} />}
...
function Dashboard({ totals, items, greeting, voiceText, listening, startVoice }: { totals: { units: number; value: number; low: number; empty: number }; items: Item[]; greeting: string; voiceText: string; listening: boolean; startVoice: () => void }) {
  const categoryData = useMemo(() => {
    const grouped = items.reduce<Record<string, number>>((result, item) => {
      result[item.group] = (result[item.group] ?? 0) + item.stock;
      return result;
    }, {});
    return Object.entries(grouped).map(([name, value]) => ({ name, value, color: groupColors[name] ?? "#8a94a6" }));
  }, [items]);
  ...
  return <div className="page-content"><section className="welcome-row"><div><h2>{greeting}, Dimas</h2><p>Berikut kondisi persediaan operasional F&amp;B hari ini.</p></div><div className="live-pill"><span /> Data demo HORECA aktif</div></section>
    ...
    <section className="dashboard-grid"><article className="panel movement-panel"><PanelTitle title="Pergerakan stok" subtitle="Barang masuk dan keluar selama 7 hari" /><div className="chart-wrap"><ResponsiveContainer width="100%" height="100%"><AreaChart data={movementData}><defs><linearGradient id="incoming" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#6d5dfc" stopOpacity={.28}/><stop offset="95%" stopColor="#6d5dfc" stopOpacity={0}/></linearGradient></defs><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="var(--line)"/><XAxis dataKey="day" axisLine={false} tickLine={false} tick={{fill:"var(--muted)",fontSize:12}}/><YAxis axisLine={false} tickLine={false} tick={{fill:"var(--muted)",fontSize:12}}/><Tooltip contentStyle={{borderRadius:14,border:"1px solid var(--line)",background:"var(--panel)",color:"var(--text)"}}/><Area type="monotone" dataKey="incoming" stroke="#6d5dfc" strokeWidth={3} fill="url(#incoming)"/><Area type="monotone" dataKey="outgoing" stroke="#ffb547" strokeWidth={2.5} fill="transparent"/></AreaChart></ResponsiveContainer></div><div className="legend"><span><i className="purple-dot"/>Barang masuk</span><span><i className="orange-dot"/>Barang keluar</span></div></article>
      <article className="panel category-panel"><PanelTitle title="Komposisi stok" subtitle="Berdasarkan kelompok inventaris" /><div className="donut-wrap"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={categoryData} dataKey="value" innerRadius={58} outerRadius={84} paddingAngle={4}>{categoryData.map((entry) => <Cell key={entry.name} fill={entry.color}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer><div className="donut-center"><strong>100%</strong><span>stok</span></div></div><div className="category-list">{categoryData.map((item) => <div key={item.name}><span><i style={{background:item.color}}/>{item.name}</span><b>{categoryTotal ? Math.round((item.value / categoryTotal) * 100) : 0}%</b></div>)}</div></article>
...
function Inventory({ items, query, setQuery, onAdd }: { items: Item[]; query: string; setQuery: (value:string)=>void; onAdd:()=>void }) {
  return <div className="page-content"><section className="page-heading"><div><h2>Daftar persediaan F&amp;B</h2><p>Kelola bahan, kemasan, supplier, lokasi, dan batas minimum stok.</p></div><button className="primary" onClick={onAdd}><Plus size={18}/> Tambah barang</button></section><article className="panel table-panel"><div className="table-toolbar"><label className="search wide"><Search size={17}/><input value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Cari barang, SKU, kategori, atau supplier…"/></label><button className="secondary"><FileDown size={17}/> Export CSV</button></div><div className="table-scroll"><table><thead><tr><th>Barang</th><th>Kelompok &amp; kategori</th><th>Supplier</th><th>Lokasi</th><th>Stok</th><th>Harga estimasi</th><th>Status</th></tr></thead><tbody>{items.map((item)=><tr key={item.id}><td><div className="item-name"><span><Package size={17}/></span><div><strong>{item.name}</strong><small>{item.sku}</small></div></div></td><td><div className="category-cell"><strong>{item.group}</strong><small>{item.category}</small></div></td><td>{item.supplier}</td><td>{item.warehouse}</td><td><b>{item.stock}</b> {item.unit}</td><td>{money.format(item.price)}</td><td><span className={item.stock===0?"status out":item.stock<=item.minimum?"status low":"status safe"}>{item.stock===0?"Habis":item.stock<=item.minimum?"Stok rendah":"Tersedia"}</span></td></tr>)}</tbody></table></div></article></div>;
}
