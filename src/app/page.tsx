  ...
  return (
    <div className={dark ? "app dark" : "app"}>
      ...
      <main className="main-shell">
        ...
        {active === "inventory" && <Inventory items={filteredItems} query={query} setQuery={setQuery} onAdd={() => setShowAdd(true)} onSelect={setSelectedItem} />}
...
function Inventory({ items, query, setQuery, onAdd, onSelect }: { items: Item[]; query: string; setQuery: (value:string)=>void; onAdd:()=>void; onSelect:(item:Item)=>void }) {
  return <div className="page-content"><section className="page-heading"><div><h2>Daftar persediaan F&amp;B</h2><p>Kelola bahan, kemasan, supplier, lokasi, dan kebutuhan restock.</p></div><button className="primary" onClick={onAdd}><Plus size={18}/> Tambah barang</button></section><article className="panel table-panel"><div className="table-toolbar"><label className="search wide"><Search size={17}/><input value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Cari barang, SKU, kategori, atau supplier…"/></label><button className="secondary" onClick={() => exportInventory(items)}><FileDown size={17}/> Export CSV</button></div><div className="table-scroll"><table><thead><tr><th>Barang</th><th>Kelompok &amp; kategori</th><th>Supplier</th><th>Lokasi</th><th>Stok</th><th>Harga estimasi</th><th>Kebutuhan</th><th>Saran restock</th><th>Aksi</th></tr></thead><tbody>{items.map((item)=>{const health=getStockHealth(item);return <tr key={item.id}><td><div className="item-name"><span><Package size={17}/></span><div><strong>{item.name}</strong><small>{item.sku}</small></div></div></td><td><div className="category-cell"><strong>{item.group}</strong><small>{item.category}</small></div></td><td>{item.supplier}</td><td>{item.warehouse}</td><td><b>{item.stock}</b> {item.unit}</td><td>{money.format(item.price)}</td><td><span className={`status ${health.tone}`}>{health.label}</span></td><td>{health.restock ? <b>{health.restock} {item.unit}</b> : "—"}</td><td><button className="table-action" onClick={() => onSelect(item)}>Detail</button></td></tr>})}</tbody></table></div></article></div>;
}
