...
type Props = {
  ...
  onUpdateRecords: (records: StockTransaction[]) => void;
};

export function TransactionsAdvanced({ items, records, onRecord, onUpdateRecords }: Props) {
  ...
  const [form, setForm] = useState({ itemId: items[0]?.id ?? "", qty: "", date: new Date().toISOString().slice(0,10), party: "", note: "", user: "Dimas Riyanto" });
  ...
  function submit(event: React.FormEvent) {
    ...
    const record = { id: `${mode === "in" ? "GRN" : "ISS"}-${Date.now().toString().slice(-8)}`, date: form.date, itemId: selected.id, item: selected.name, type: mode, qty: Number(form.qty), unit: selected.unit, party: form.party || (mode === "in" ? selected.supplier : "Operasional"), note: form.note || "Tanpa catatan", user: form.user } as StockTransaction & { user: string };
    const error = onRecord(record); if (error) { setMessage(error); return; }
    setMessage("Transaksi berhasil disimpan."); setForm((current) => ({ ...current, qty: "", party: "", note: "" }));
  }

  function remove(id: string) {
    if (!window.confirm("Hapus transaksi ini? Stok saat ini tidak akan dihitung ulang otomatis karena mode percobaan.")) return;
    onUpdateRecords(records.filter((record) => record.id !== id));
  }
  ...
  function edit(record: StockTransaction) {
    ...
    onUpdateRecords(records.map((entry) => entry.id === record.id ? { ...entry, qty, note } : entry));
  }
  ...
  return <div className="page-content">
    ...
    <section className="transaction-workspace"><form className="panel entry-form" onSubmit={submit}><div className="section-title"><div><h3>Transaksi baru</h3><p>Semua data tetap disimpan lokal untuk mode percobaan.</p></div></div><div className="form-grid">
      ...
      <label>Jumlah<input type="number" min="1" required value={form.qty} onChange={(e)=>setForm({...form,qty:e.target.value})}/></label>
      <label>Satuan<input disabled value={selected?.unit ?? "-"}/></label>
    ...
    <article className="panel history-panel"><div className="transaction-filter-grid"><label className="search"><Search size={16}/><input value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Cari transaksi…"/></label><select value={type} onChange={(e)=>setType(e.target.value as typeof type)}><option value="all">Semua jenis</option><option value="in">Barang masuk</option><option value="out">Barang keluar</option></select><select value={itemId} onChange={(e)=>setItemId(e.target.value)}><option value="all">Semua barang</option>{items.map((item)=><option key={item.id} value={item.id}>{item.name}</option>)}</select><input type="date" value={from} onChange={(e)=>setFrom(e.target.value)}/><input type="date" value={to} onChange={(e)=>setTo(e.target.value)}/></div>
      <div className="table-scroll"><table><thead><tr><th>Tanggal</th><th>Barang</th><th>Jenis</th><th>Jumlah</th><th>Pihak</th><th>Pengguna</th><th>Catatan</th><th>Aksi</th></tr></thead><tbody>{filtered.map((record)=><tr key={record.id}><td>{record.date}</td><td><strong>{record.item}</strong><small className="table-meta">{record.id}</small></td><td>{record.type === "in" ? "Masuk" : "Keluar"}</td><td>{record.qty} {record.unit}</td><td>{record.party}</td><td>{(record as StockTransaction & {user?:string}).user ?? "Administrator"}</td><td>{record.note}</td><td><div className="row-actions"><button onClick={()=>edit(record)}><Pencil size={14}/></button><button onClick={()=>remove(record.id)}><Trash2 size={14}/></button></div></td></tr>)}</tbody></table></div>
    </article>
  </div>;
}
...
export function StocktakeAdvanced({ items, onApply }: { items: Item[]; onApply: (counts: Record<string, number>) => void }) {
  ...
  const [history, setHistory] = useState<OpnameRecord[]>(() => { if (typeof window === "undefined") return []; try { return JSON.parse(localStorage.getItem("stockflow-opname-history-v2") || "[]"); } catch { return []; } });
